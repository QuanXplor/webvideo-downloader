...
def request(*args, **kargs):
    ...
    reqLogger and reqLogger.debug('%s %s' % (args, kargs))
...
def acquireDiskFileSize(filePath):
...
class VideoInfo:
    def __init__(self, filePath):
        ...
        for stream in metadata.streams:
            if stream.is_video():
                self.duration=int(stream.duration_seconds())
                self.resolution = "{0:.0f}×{1:.0f}".format(*stream.frame_size())

    def durationFormat(self):
        return durationFormat(self.duration)
